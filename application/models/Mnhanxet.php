...
class Mnhanxet extends MY_Model
{
	...
	public function getNhanXet($maUngVien)
	{		
		$this->db->where('FK_iMaUV', $maUngVien);
		return $this->db->get('tbl_nhanxet')->row_array();
    }
	public function saveNhanXet($maUngVien, $arrSave)
	{
		$this->db->where('FK_iMaUV', $maUngVien);
		$this->db->delete('tbl_nhanxet');
		$this->db->insert('tbl_nhanxet', $arrSave);
