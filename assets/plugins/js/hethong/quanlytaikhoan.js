...
$(document).ready(function () {
	...
	$(document).on('click', 'button[name=btnFix]', function () {
		...
		$.ajax({
			...
			success: function(response){
				...
				if(info['FK_iMaNganh']!=null) {
					$('#modalFix .nganh-form').removeClass('hidden');
					$('#modalFix').find('select[name=maNganh]').val(info['FK_iMaNganh']+'|'+info['sChuyenNganh']);
					console.log(info['FK_iMaNganh']+'|'+info['sChuyenNganh']);
				}
				else {
					$('#modalFix .nganh-form').addClass('hidden');
					$('#modalFix').find('select[name=maNganh]').val('');
				}
	...
	$('select[name=maQuyen]').on('change', function () {
		if($(this).val()==4||$(this).val()==5) {
			$('.nganh-form').removeClass('hidden');
		}
		else {
			$('.nganh-form').addClass('hidden');
			$('select[name=maNganh]').val('');
			console.log($('select[name=maNganh]').val());
		}
	});
});